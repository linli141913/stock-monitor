...
- 盘中特征明细保留60个交易日，最短工程影子观察20个交易日；日频事实和候选摘要长期保留，不长期保存上游响应正文；
...
- 新增6项策略专项测试，覆盖默认禁用规则、权重/阈值完整性、100%行业映射门槛、逐项准入原因、60日/20日保留策略和低频任务阻断；
- 阶段5及既有雷达相关166项测试通过；
- Python语法编译和 `git diff --check` 通过；
