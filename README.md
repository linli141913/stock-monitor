...
阶段10已提供趋势轮动、ETF监测和龙头观察三模块的只读正式就绪控制面：各模块分别展示真实影子观察、阶段9质量与九项自动证据门。新增的离线采集入口会把当日运行回执、真实来源工件、外部冻结新鲜度策略和上交所官方休市原文交叉校验后，再幂等登记到内容寻址的 v2 影子台账。统一现场协调器只消费内容寻址的冻结调度清单，并在任何外部请求前发布不可变 `pending` 检查点和实际调用全集；失败、锁竞争、恢复和同一时隙的额外调用都不能被隐藏。运营证据从该全集、历史台账、冻结策略和静态资产自动重放，手工输入不能自报全绿。趋势模块还要求阶段6生成器原生落盘完整市场特征快照及独立哈希，采集器会重建强类型快照并用冻结规则重算市场状态，不信任上游自报的标签、批次或哈希；ETF准入只证明产品身份，不能代替当日现场观察。正式就绪报告必须显式携带版本化有效期策略，API和调度守卫在每次读取、注册和执行时都会用服务端时钟重验报告、普通证据和运营证据年龄；缺策略或过期都不能继续使用旧的全绿结果。它不等于正式任务已经接线或任何模块已经开启；`ready_to_enable` 仍需独立生产授权，页面不提供股票审批或自动交易。

- [阶段10正式启用运行手册](docs/阶段10正式启用运行手册.md)：只读检查、离线演练、授权、停止和回退边界。
- [用户使用手册](docs/用户使用手册.md)：状态、时间、来源、缺失和 AI 的阅读方式。
...
`status` 不因LaunchAgent已加载就承诺运行正常：ngrok必须同时满足LaunchAgent加载、PID可验证和4040端口监听三个信号才是 `running|identity_verified`；任意信号冲突都是 `degraded|identity_unverified`。实时结论只以当次只读输出为准，不把某个PID或端口状态写成长期保证。`preflight` 会因正式服务已存在而拒绝重复安装；`install`、`uninstall` 和 `rollback-screen` 会修改本机服务状态，只能在单独授权后执行。
